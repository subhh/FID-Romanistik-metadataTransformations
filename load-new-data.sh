...
== examples ==
./load-new-data.sh -c dialnet-tesis -i http://dialnet.unirioja.es/oaites/OAIHandler -d http://localhost:3333
./load-new-data.sh -c dialnet-articulos -i http://dialnet.unirioja.es/oai/OAIHandler -d http://localhost:3333
EOF
   exit 1
}
